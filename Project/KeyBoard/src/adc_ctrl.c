...
static StRockState s_stMiniRockState;
...
void RockPushRodInit(void)
{
	...
	memset(&s_stMiniRockState, 0, sizeof(StRockState));
...
static bool MiniRockGetValue(StRockState *pRockState)
{
	u32 i;
	...
	for (i = 0; i < 2; i++)
	{
		u16RockValue = ADCGetAverage(i + MINI_ROCK_CHANNEL);
...
static bool RockGetValue(StRockState *pRockState)
{
	...
	for (i = 0; i < 3; i++)
	{
		u16RockValue = ADCGetAverage(i + ROCK_CHANNEL);
...
{
	if (MiniRockGetValue(&s_stMiniRockState))
	{
		StKeyMixIn stKey;

		stKey.emKeyType = _Key_MiniRock;
		memcpy(&(stKey.unKeyMixIn.stMiniRockState), &s_stMiniRockState, 
			sizeof(StRockState));
		KeyBufWrite(&stKey);
	}
}
...
#define PR_CTRL_KEY			_Key_Overlay_1
#define PR_CTRL_LED			_Led_Overlay_1
